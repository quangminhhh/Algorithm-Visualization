...
class SortingVisualizer {
    ...
    highlightBars(indices_1, className_1) {
        return __awaiter(this, arguments, void 0, function* (indices, className, duration = 300) {
            const bars = document.querySelectorAll('.bar');
            indices.forEach(i => {
                if (bars[i])
                    bars[i].classList.add(className);
            });
            yield this.sleep(duration); // Use the dynamic speed
            indices.forEach(i => {
                if (bars[i])
                    bars[i].classList.remove(className);
            });
            this.recordStep();
            yield this.checkPaused();
        });
    }
    swapBars(i, j) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!bars[i] || !bars[j])
                return;
            yield this.highlightBars([i, j], 'bar-swapping', this.speed);
            // Swap in array
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < n - 1; i++) {
                ...
                for (let j = 0; j < n - i - 1; j++) {
                    ...
                    if (!this.isRunning)
                        return;
                    yield this.highlightBars([j, j + 1], 'bar-comparing', this.speed);
                    this.updateStats('comparisons');
                    this.highlightPseudo(2);
                    if (this.array[j] > this.array[j + 1]) {
                        this.highlightPseudo(3);
                        yield this.swapBars(j, j + 1);
                    }
                }
                this.highlightPseudo(4);
                yield this.highlightBars([n - i - 1], 'bar-sorted', 100); // Mark as sorted
            }
            if (n > 0)
                yield this.highlightBars([0], 'bar-sorted', 100); // Mark the last element if exists
    ...
    selectionSort() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < n - 1; i++) {
                ...
                for (let j = i + 1; j < n; j++) {
                    this.highlightPseudo(2);
                    yield this.highlightBars([j, minIdx], 'bar-comparing', this.speed);
                    this.updateStats('comparisons');
                    this.highlightPseudo(3);
                ...
                if (minIdx !== i) {
                    ...
                    yield this.swapBars(i, minIdx);
                }
                yield this.highlightBars([i], 'bar-sorted', 100);
            }
            if (n > 0)
                yield this.highlightBars([n - 1], 'bar-sorted', 100);
            this.clearPseudoHighlight();
        });
    }
    insertionSort() {
        return __awaiter(this, void 0, void 0, function* () {
            const n = this.array.length;
            if (n > 0)
                yield this.highlightBars([0], 'bar-sorted', 100); // First element is trivially sorted
            for (let i = 1; i < n; i++) {
                if (!this.isRunning)
                    return;
                this.highlightPseudo(0);
                ...
                yield this.highlightBars([i], 'bar-comparing', this.speed);
                while (j >= 0 && this.array[j] > key) {
                    ...
                    if (bars[j + 1]) {
                        ...
                        if (bars[j + 1]._tippy)
                            bars[j + 1].setAttribute('data-tippy-content', `Value: ${this.array[j + 1]}, Index: ${j + 1}`);
                    }
                    j--;
                ...
                if (bars[j + 1]) {
                    ...
                    if (bars[j + 1]._tippy)
                        bars[j + 1].setAttribute('data-tippy-content', `Value: ${key}, Index: ${j + 1}`);
                }
                // Mark all elements up to i as sorted (conceptually)
                for (let k = 0; k <= i; k++) {
                    yield this.highlightBars([k], 'bar-sorted', 50);
                }
            }
            this.clearPseudoHighlight();
        });
    ...
    mergeSort() {
        return __awaiter(this, arguments, void 0, function* (left = 0, right = this.array.length - 1) {
            if (!this.isRunning)
                return;
            if (left >= right) {
                if (left >= 0 && left < this.array.length)
                    yield this.highlightBars([left], 'bar-sorted', this.speed / 2);
                return;
            }
    ...
    merge(left, mid, right) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (i < leftArr.length && j < rightArr.length) {
                ...
                yield this.highlightBars([left + i, mid + 1 + j, k], 'bar-comparing', this.speed);
                this.updateStats('comparisons');
                ...
                if (bars[k]) {
                    ...
                    if (bars[k]._tippy)
                        bars[k].setAttribute('data-tippy-content', `Value: ${this.array[k]}, Index: ${k}`);
                }
                yield this.highlightBars([k], 'bar-swapping', this.speed / 2); // Show placement
            ...
            while (i < leftArr.length) {
                ...
                yield this.highlightBars([k], 'bar-swapping', this.speed / 2);
            ...
            for (let l_idx = left; l_idx <= right; l_idx++) {
                yield this.highlightBars([l_idx], 'bar-sorted', 50);
            }
        });
    }
    quickSort() {
        return __awaiter(this, arguments, void 0, function* (low = 0, high = this.array.length - 1) {
            if (!this.isRunning)
                return;
            if (low < high) {
                const pi = yield this.partition(low, high);
                yield this.highlightBars([pi], 'bar-sorted', 100); // Pivot is in place
                yield this.quickSort(low, pi - 1);
                yield this.quickSort(pi + 1, high);
            }
            else if (low === high && low >= 0 && low < this.array.length) {
                yield this.highlightBars([low], 'bar-sorted', 100); // Single element is sorted
            }
        });
    }
    partition(low, high) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.highlightBars([high], 'bar-comparing', this.speed); // Highlight pivot
            let i = low - 1; // Index of smaller element
            for (let j = low; j < high; j++) {
                if (!this.isRunning)
                    return i + 1; // Exit if stopped
                yield this.highlightBars([j, high], 'bar-comparing', this.speed);
                this.updateStats('comparisons');
                if (this.array[j] < pivot) {
                    i++;
                    if (i !== j) { // Avoid swapping with itself
                        yield this.swapBars(i, j);
                    }
                }
            }
            // Swap pivot to its correct position
            if ((i + 1) !== high) { // Avoid swapping with itself
                yield this.swapBars(i + 1, high);
            }
    ...
    heapSort() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = n - 1; i > 0; i--) {
                if (!this.isRunning)
                    return;
                yield this.swapBars(0, i);
                yield this.highlightBars([i], 'bar-sorted', 100); // Element is now sorted
                yield this.heapify(i, 0); // Heapify the reduced heap
            }
            if (n > 0)
                yield this.highlightBars([0], 'bar-sorted', 100); // Last element
        });
    }
    heapify(n, i) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const toHighlight = [i];
            if (left < n)
                toHighlight.push(left);
            if (right < n)
                toHighlight.push(right);
            yield this.highlightBars(toHighlight, 'bar-comparing', this.speed / 2);
            if (left < n) {
                this.updateStats('comparisons');
            ...
            if (largest !== i) {
                yield this.swapBars(i, largest);
                yield this.heapify(n, largest); // Recursively heapify the affected sub-tree
            }
        });
    }
    startSorting() {
            ...
            try {
                switch (algorithm) {
                    ...
                    case 'insertion':
                        ...
                        break;
                    case 'merge':
                        yield this.mergeSort();
                        // Final pass to ensure all are marked sorted for merge sort
                        if (this.isRunning)
                            yield this.highlightAllSorted();
                        break;
                    case 'quick':
                        yield this.quickSort();
                ...
                if (this.isRunning) { // If not reset during execution
                    yield this.celebrateCompletion();
                }
            }
            catch (error) {
    ...
    celebrateCompletion() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < bars.length; i++) {
                ...
                yield this.sleep(Math.max(5, 50 - bars.length / 2)); // Faster for more bars
            }
            // Fireworks effect
            ...
            yield this.sleep(bars.length * Math.max(5, 50 - bars.length / 2) + 500); // Wait for rainbow + fireworks
    ...
    highlightAllSorted() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < this.array.length; i++) {
                if (!this.isRunning)
                    return;
                yield this.highlightBars([i], 'bar-sorted', Math.max(10, 50 - this.array.length / 2));
            }
        });
    }
    resetArray() {
                                ...
                                afterBuildTicks: chart => {
                                    ...
                                    while (currentTick <= yMax) {
                                        ...
                                        if (currentTick < 10)
                                            ...
                                            currentTick += 100;
                                        else if (currentTick < 10000)
                                            currentTick += 1000;
                                        else if (currentTick < 100000)
                                            currentTick += 10000;
                                        else
                                            currentTick += 100000;
                                        if (ticks.length > 20 || currentTick > 2000000)
                                            break; // Limit number of ticks and prevent infinite loops
                                    }
    ...
    checkPaused() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.isPaused) {
                yield new Promise(resolve => this.pauseResolver = resolve);
                this.pauseResolver = null;
                if (this.stepMode)
                    this.isPaused = true;
            }
    ...
    showSnapshot(snapshot) {
        ...
        snapshot.forEach((val, idx) => {
            if (bars[idx]) {
                ...
                if (bars[idx]._tippy)
                    bars[idx].setAttribute('data-tippy-content', `Value: ${val}, Index: ${idx}`);
            }
        });
    }
    pause() {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function playTone(frequency, duration, type = 'sine', volume = 0.05) {
        ...
        if (audioContext.state !== 'running')
            return; // Don't play if context isn't running
    ...
    soundToggle.addEventListener('click', () => {
        ...
        if (soundEnabled)
            playTone(440, 100); // Play a test tone
    });
    ...
    visualizer.swapBars = function (i, j) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield originalSwapBars(i, j);
        });
    };
    const originalHighlightBars = visualizer.highlightBars.bind(visualizer);
    visualizer.highlightBars = function (indices, className, duration) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield originalHighlightBars(indices, className, duration);
        });
    };
